...
const AssignedForm = () => {
  ...
  const handleSubmit = async () => {
    if (!selectedItem || !assignedTo || !count || !assignmentDate || !returnDate) {
      toast.dismiss();
    ...
    if (assignDate < today) {
      toast.dismiss();
      toast.error("Assignment date cannot be in the past", { toastId: "assign-date" });
      return;
    }

    if (returnDt < today) {
      toast.dismiss();
      toast.error("Return date cannot be in the past", { toastId: "return-date" });
      return;
    }

    if (returnDt <= assignDate) {
      toast.dismiss();
      toast.error("Return date must be after assignment date", { toastId: "date-order" });
      return;
    }
    ...
    try {
      setLoading(true);
      await axios.post(`${import.meta.env.VITE_API_URL}/api/inventory/assigned`, payload, {
